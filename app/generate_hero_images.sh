...
HERO_IMAGE_DIR="public/hero_images"
...
main () {
  ...
  find -E ${SOURCE_IMAGE_DIR} -depth 1 -regex '.*(png|jpg|jpeg|webp)' -print |while read file; do
    ...
    generate_desktop_images ${file} ${HERO_IMAGE_DIR}/${IMAGE_NAME}

    # Use the previously-optimized desktop image to generate the blurred and mobile images.
    generate_blurred_images ${HERO_IMAGE_DIR}/${IMAGE_NAME} ${BLURRED_IMAGE_DIR}/${IMAGE_NAME} &
    generate_mobile_images ${HERO_IMAGE_DIR}/${IMAGE_NAME} ${MOBILE_IMAGE_DIR}/${IMAGE_NAME} &
  done
}
...
if [[ $1 != "dev" ]]; then
  optimize_images ${HERO_IMAGE_DIR}
fi

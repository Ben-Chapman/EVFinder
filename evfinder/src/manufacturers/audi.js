...
import { generateErrorMessage, getGeoFromZipcode, stripHTML, titleCase } from '../helpers/libs'
...
function formatAudiVinResults(input) {
  ...
  let techSpecs = []
  Object.keys(input.data.getVehicleInfoForWormwood.technicalSpecifications).forEach((key) => {
   ...
   */
  Object.keys(input.data.getVehicleInfoForWormwood.equipments).forEach((equipmentType) => {
    if (equipmentType != '__typename') {
      let equipment = []
      // Optional equipment
      if (equipmentType == 'optionalEquipments') {
        input.data.getVehicleInfoForWormwood.equipments[equipmentType].forEach((e) => {
          equipment.push(e['headline'])
        })
      }
      // Standard equipment
      else if (equipmentType == 'standardEquipments') {
        Object.keys(input.data.getVehicleInfoForWormwood.equipments[equipmentType]).forEach((e) => {
          if (
            e != '__typename' &&
            input.data.getVehicleInfoForWormwood.equipments[equipmentType][e] !== null &&
            input.data.getVehicleInfoForWormwood.equipments[equipmentType][e].length > 0
          ) {
            Object.keys(input.data.getVehicleInfoForWormwood.equipments[equipmentType][e]).forEach(
              (key) => {
                const value = input.data.getVehicleInfoForWormwood.equipments[equipmentType][e][key]

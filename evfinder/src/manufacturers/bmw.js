...
function formatBMWInventoryResults(input) {
  ...
  input?.data?.getInventory?.result.forEach((vehicle) => {
    ...
    Object.keys(vehicle).forEach((key) => {
      ...
      vehicle['distance'] = parseFloat(vehicle['distanceToLocatorZip']).toFixed(2).toString()
      ...
      tmp['dealerUrl'] = dealerDetail.newVehicleSales[0].dealerURL.replace('https://', '')
...
export async function getBMWVinDetail(vin, manufacturer, inventoryData) {
  try {
    ...
    vinFormattedData['Dealer Estimated Arrival Date'] = inventoryData?.dealerEstArrivalDate
